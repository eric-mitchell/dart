...
def get_mnist_data(device, batch_size: int = 128):
...
def train(args: argparse.Namespace, model: DART):
    ...
    train_loader, (X_test, y_test) = get_mnist_data(args.device, args.batch_size)
    ...
    for epoch in range(args.epochs):
        for t, (X, _) in enumerate(train_loader):
            ...
            if t % 100 == 0:
                ...
                test_log_px = model.log_px(test_data)
...
def run(args: argparse.Namespace):
    ...
    dart = DART(784, args.hidden_size, args.n_hidden, args.alpha_dim)
